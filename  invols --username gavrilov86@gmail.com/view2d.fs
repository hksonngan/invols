...
void main()
{
	float val = params.x ? interpolate_bicubic(txt, vertex.xy) : texture2D(txt, vertex.xy).x;
	vec3 color = params.y ? GetLevelColor(val).xyz : vec3((val - min_level)/(max_level-min_level));
